...
// Stable per-class id so re-exporting / re-syncing updates rather than
// duplicates. Prefers the portal's own record id; falls back to date/time for
// any entry that somehow arrives without one.
...
// Title with quiz / exam / mandatory (★) / session (S<n>:) prefixes. Quiz and
// exam prefixes win over ★; quiz wins over exam.
...
// faculty / session / [Mandatory], newline-joined. (The TCS iON version led with
// the short code; the portal doesn't send one.)
function eventDescription(cls) {
  const mandatory = isMandatory(cls) || isQuiz(cls) || isExam(cls);
  return [cls.faculty, sessionLabel(cls), mandatory ? "[Mandatory]" : ""]
    .filter(Boolean)
    .join("\n");
}
...
function buildGoogleEvents(classes) {
  ...
  for (const cls of classes) {
    ...
    const event = {
      ...
      // Portal times are IST — pin the zone so Google places them correctly.
    ...
    // Quizzes & exams → tomato (red); other mandatory classes → banana (yellow).
